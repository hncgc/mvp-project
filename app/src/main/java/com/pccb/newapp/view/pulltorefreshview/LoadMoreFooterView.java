...
public class LoadMoreFooterView extends LinearLayout {
	...
	private void initView(Context context) {
		...
		footerView.setMinimumHeight(ViewUtil.scale(mContext,100));
		footerTextView = new TextView(context);  
		footerTextView.setGravity(Gravity.CENTER_VERTICAL);
		setTextColor(Color.rgb(107, 107, 107));
		ViewUtil.setTextSize(footerTextView,24);
		ViewUtil.setPadding(footerView, 0, 10, 0, 10);
		
		LayoutInflater inflater = (LayoutInflater)context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
		footerProgressBar = (ProgressWheel) inflater.inflate(R.layout.loading_view, null).findViewById(R.id.loading_view_rw);
		footerProgressBar.setBarWidth(2);
		footerProgressBar.setVisibility(View.GONE);
		
		LayoutParams layoutParamsWW = new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
		layoutParamsWW.gravity = Gravity.CENTER;
		layoutParamsWW.width = ViewUtil.scale(mContext, 40);
		layoutParamsWW.height = ViewUtil.scale(mContext, 40);
